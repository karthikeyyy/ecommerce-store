...
const EditProduct = () => {
  ...
  useEffect(() => {
    const loadAll = async () => {
      try {
        const [catRes, tagRes, attrRes, prod] = await Promise.all([
        ...
        const categoryIds = Array.isArray(prod.category)
          ? prod.category.map((c) => (typeof c === "string" ? c : c._id))
          : [];

        const tagIds = Array.isArray(prod.tags)
          ? prod.tags.map((t) => t._id)
          : [];
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500 dark:text-slate-400">
                  Update product details, pricing, media, and categories.
                </p>
              </div>
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="px-5 py-2 rounded-xl bg-blue-500 text-sm font-semibold text-white shadow-md hover:bg-blue-400 transition disabled:opacity-70"
                  disabled={saving}
                >
                        ...
                        return (
                          <div
                            ...
                            className={`rounded-xl border p-4 transition ${isAttrSelected
                              ? "border-blue-500 bg-blue-50/50 dark:border-blue-500/50 dark:bg-blue-500/5"
                          ...
                          >
                            <label className="flex items-center gap-3 cursor-pointer">
                              <input
                                type="checkbox"
                                className="h-5 w-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900"
                                ...
                                {attr.values.map((val) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      <input
                                        type="checkbox"
                                        className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900"
...
const TextField = ({ label, value, onChange, placeholder }) => (
  <div className="space-y-1.5">
    ...
    <input
      ...
      className="w-full rounded-xl border border-gray-300 bg-white px-3 py-2.5 text-sm text-gray-900 outline-none ring-0 transition placeholder:text-gray-400 focus:border-blue-500 dark:border-slate-700 dark:bg-slate-950/60 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:border-blue-500"
    />
  </div>
);
...
const SelectField = ({ label, value, onChange, options }) => (
  <div className="space-y-1.5">
    ...
    <select
      value={value}
      onChange={onChange}
      className="w-full rounded-xl border border-gray-300 bg-white px-3 py-2.5 text-sm text-gray-900 outline-none ring-0 transition focus:border-blue-500 dark:border-slate-700 dark:bg-slate-950/60 dark:text-slate-100 dark:focus:border-blue-500"
    >
      {options.map((opt) => (
