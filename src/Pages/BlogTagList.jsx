...
const BlogTagList = () => {
  ...
  const fetchTags = async () => {
    try {
      const response = await apiRequest("/blog-tags");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (currentTag) {
        await apiRequest(`/blog-tags/${currentTag._id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
      } else {
        await apiRequest("/blog-tags", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this blog tag?")) {
      try {
        const response = await apiRequest(`/blog-tags/${id}`, {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
        });
  ...
  const handleBulkDelete = async () => {
    ...
    if (
      window.confirm(
        `Are you sure you want to delete ${selectedTags.length} blog tags?`
      )
    ) {
      try {
        await apiRequest("/blog-tags/bulk-delete", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleBulkUpdateStatus = async (isActive) => {
    if (selectedTags.length === 0) return;
    try {
      await apiRequest("/blog-tags/bulk-status", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      console.error("Error updating blog tag status:", error);
    }
  };

  const handleToggleStatus = async (id, currentStatus) => {
    try {
      await apiRequest(`/blog-tags/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
    ...
    <>
          ...
          <main className="flex-1 px-4 py-6 md:px-8 md:py-8">
            <div className="max-w-7xl mx-auto space-y-6">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                  <h1 className="text-3xl font-semibold tracking-tight">Blog Tags</h1>
                  <p className="mt-1 text-sm text-gray-500 dark:text-slate-400">Manage tags used in blog posts</p>
                </div>
                            ...
                            filteredTags.map((tag) => (
                              ...
                              >
                                ...
                                <td className="py-3 px-4">
                                  ...
                                  >
                                    <span
                                      className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${tag.isActive
                                        ? "translate-x-6"
                                        : "translate-x-1"
                                        }`}
                                    />
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-xl max-h-[90vh] overflow-y-auto border border-gray-200 dark:border-slate-800">
